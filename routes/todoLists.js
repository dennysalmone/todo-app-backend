...
router.delete('/delete-list', async (req, res) => {
    ...
    try {
        await Board.updateOne({id: req.body.boardId, access: req.headers["email"]}, {$pull : { lists : { collectionId : req.body.collId } } } );
...
router.post('/todo-list', async (req, res) => {
    ...
    let board = await Board.findOne({id: req.body.boardId, access: req.headers["email"]});
